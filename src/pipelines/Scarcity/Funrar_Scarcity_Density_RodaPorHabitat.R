...
## The way that funrar works is that it is based on matrices. These matrices
## only accept one character variable. So we need to choose the site level to be
## used. Here, I will use at habitat level.
...
# Create the df with the habitat names.
...
# variables.
rownames(df_matrix) <- paste0(df_clean_samples$habitat)
...
# The matrix can be in stack format
...
quant <- quantile(si_df$Si, probs = seq(0, 1))  #Calculate quantiles

labels_quant <- paste(names(quant)[-length(quant)], names(quant)[-1], sep = "-") #Define labels
...
si_df <- merge(si_df, df_char, by = "habitat")
...
### -----------------------Essa parte provavelmente vai sair------------------------------------#
...
write.csv(si_spread,
          file = paste("Outputs/Tables/scarcity_index-taxon_summarized_habitat",d,".csv"),
          row.names = FALSE)
